            ...
            packages.webdocs =
              let
                nim-jester = pkgs.stdenv.mkDerivation {
                  ...
                  src = pkgs.fetchFromGitHub {
                    ...
                    repo = "jester";
                    rev = "v0.6.0";
                    sha256 = "sha256-F/zWWGipJ4lBE3njceXn5HBFTYEXB4l2rk6+FfqqZTQ=";
                  };
                  dontBuild = true;
                  installPhase = ''
                    mkdir -p $out/lib
              ...
              pkgs.stdenv.mkDerivation {
                ...
                configurePhase = ''
                  ...
                  ${pkgs.nim2}/bin/nim doc -o:docout/api/yaml.html --docSeeSrcUrl:https://github.com/flyx/NimYAML/blob/${self.rev or "master"} yaml
                  for srcFile in yaml/*.nim; do
                    bn=''${srcFile#yaml/}
                    ${pkgs.nim2}/bin/nim doc -o:docout/api/yaml/''${bn%.nim}.html --docSeeSrcUrl:https://github.com/flyx/NimYAML/blob/yaml/${self.rev or "master"} $srcFile
                  done
                '';
                buildPhase = ''
          ...
          config = mkIf cfg.enable {
            systemd.services.nimyaml-webdocs = {
              ...
              serviceConfig = {
                Restart = "on-failure";
                RestartSec = "5s";
                ExecStart = ''${webdocs}/bin/nimyaml-docs-server --address="${cfg.address}" --port=${toString cfg.port}'';
              };
            };
          };
        };
