...
import ../yaml/[ presenter, data, stream, dumping ]
import unittest
...
let jsonOptions = jsonDumper().presentation
...
suite "Presenter":
  ...
  test "JSON output with only scalar":
    var input = inputSingle(scalarEvent("Lorem ipsum\nDolor sit amet"))
    assertOutput(input, "\"Lorem ipsum\\nDolor sit amet\"\n", jsonOptions)

  test "JSON output with array":
    var input = inputSingle(startSeqEvent(), scalarEvent("a"), scalarEvent("b"), endSeqEvent())
    assertOutput(input, "[\n  \"a\",\n  \"b\"\n]\n", jsonOptions)

  test "JSON output with empty array":
    var input = inputSingle(startSeqEvent(), endSeqEvent())
    assertOutput(input, "[]\n", jsonOptions)

  test "JSON output with mapping":
    var input = inputSingle(
